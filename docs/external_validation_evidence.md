...
> These values are curated aggregate context used to explain the validation and adaptation workflow. They are kept separate from outputs generated by the public synthetic pipeline.

## 4. Discrimination and calibration are separate questions

> The figures in this section are public-facing aggregate summaries. They preserve the structure and high-level patterns of the private retrospective validation analysis after public-safe reduction, while the underlying clinical dataset, patient-level prediction rows, ROC coordinates, and full calibration-bin source tables remain outside the public repository.
...
The figures and compact tables in this document are aggregate evidence summaries. Patient-level predictions, internal variable mappings, source extracts, and the full private output directory remain outside the public repository.
