...
## 11. Public evidence layers

The repository separates two kinds of outputs:

- `evidence/` contains curated aggregate context from the private retrospective analysis;
- `results/` contains outputs generated by the public synthetic reconstruction.

This separation prevents synthetic demonstration results from being mistaken for clinical study estimates while still making the original evidence chain visible.

## 12. Synthetic stress-test design

The public generator is not intended to numerically clone the private cohort. It creates repeated exams, administrative censoring, horizon-specific eligibility, a fixed image-risk signal, additional local structured signal, and controlled calendar-time shift. The purpose is to demonstrate why random patient-split improvement and future-period transportability can diverge.
