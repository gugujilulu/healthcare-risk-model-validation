...
from .metrics import discrimination_calibration_metrics, logit
...
def _selection_score(metrics: dict) -> tuple:
    # Primary: AUC; secondary: Brier; tertiary: absolute E/O error.
    return (metrics["auc"], -metrics["brier"], -abs(metrics["e_o_ratio"] - 1))


def fit_locked_horizon(
    df: pd.DataFrame,
    horizon: int,
    candidate_models=("logistic", "lightgbm"),
    calibration_methods=("none", "platt", "isotonic", "beta_style"),
    seed: int = 20260812,
):
    """Select model/calibration without touching the final test set.

    Train models on train. Split validation patients into calibration-fit and
    selection subsets. Fit calibration on calibration-fit; choose the winner on
    selection. Evaluate the locked winner once on test.
    """
    ...
    selected = SelectedModel(
        ...
        estimator=winner["_estimator"],
        calibrator=winner["_calibrator"],
...
def fit_all_horizons(df: pd.DataFrame, horizons=(1, 2, 3, 4, 5), seed: int = 20260812):
    ...
    for h in horizons:
        ...
        outputs[h] = out
        om = out["original_metrics"]
        am = out["adapted_metrics"]
        summary.append(
            {
                "horizon": f"{h}yr",
                "horizon_years": h,
                ...
                "original_auc": om["auc"],
                "adapted_auc": am["auc"],
                "delta_auc": am["auc"] - om["auc"],
                "original_brier": om["brier"],
                "adapted_brier": am["brier"],
                "delta_brier": am["brier"] - om["brier"],
                "original_e_o_ratio": om["e_o_ratio"],
                "adapted_e_o_ratio": am["e_o_ratio"],
                "adapted_calibration_intercept": am["calibration_intercept"],
                "adapted_calibration_slope": am["calibration_slope"],
                "test_n": am["n"],
                "test_cases": am["cases"],
            }
        )
        ...
        pred["horizon"] = f"{h}yr"
        pred["label"] = out["y_test"]
        pred["original_mirai"] = out["p_original"]
        pred["adapted_model"] = out["p_adapted"]
        predictions.append(pred)

        st = out["selection_table"].copy()
        st.insert(0, "horizon", f"{h}yr")
        selections.append(st)
