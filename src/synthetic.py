...
def generate_synthetic_screening_data(n_patients: int = 12000, seed: int = 20260812) -> pd.DataFrame:
    ...
    for pid in range(1, n_patients + 1):
        ...
        event_latent = (-4.65 + 0.050 * (base_age - 55) + 0.65 * density_effect + 0.70 * (family == "yes") + 0.55 * (biopsy == "yes") + rng.normal(0, 0.45))
        annual_hazard = np.exp(event_latent)
        baseline_latent = (-4.55 + 0.040 * (base_age - 55) + 0.50 * density_effect + 0.12 * (family == "yes") + rng.normal(0, 0.28))
        ...
        cancer_date = first_exam_date + pd.to_timedelta(event_delay * 365.25, unit="D") if has_event else pd.NaT
        admin_followup_end = pd.Timestamp("2021-12-31")
        if rng.random() < 0.18:
            admin_followup_end = min(admin_followup_end, first_exam_date + pd.to_timedelta(rng.uniform(2.0, 7.0) * 365.25, unit="D"))

        for j in range(n_exams):
            exam_date = first_exam_date + pd.DateOffset(years=2 * j)
            if exam_date > pd.Timestamp("2019-12-31"):
                break
            if pd.notna(cancer_date) and exam_date >= cancer_date:
                break
            ...
            tte = ((cancer_date - exam_date).days / 365.25) if pd.notna(cancer_date) else np.inf
