...
def run_post_screen_triage(df: pd.DataFrame, horizons=(1, 2), seed: int = 20260812):
    ...
    for h in horizons:
        ...
        numeric = ["age", "prior_exam_count", "finding_mass", "finding_calcification", "finding_asymmetry"]
        ...
        pipe.fit(tr[POST_FEATURES], tr[ycol].astype(int))
        pv = pipe.predict_proba(va[POST_FEATURES])[:,1]
        cal = PlattCalibrator().fit(pv, va[ycol].astype(int))
        pt = cal.predict(pipe.predict_proba(te[POST_FEATURES])[:,1])
